-- Migration: CBT Canonical Identity

-- 1. Alter cbt_attempt_answers table to fix the identity flaw
ALTER TABLE public.cbt_attempt_answers 
  RENAME COLUMN question_id TO legacy_client_key;

ALTER TABLE public.cbt_attempt_answers
  ADD COLUMN question_id TEXT REFERENCES public.exam_questions(id) ON DELETE CASCADE,
  ADD COLUMN bank_question_id UUID REFERENCES public.questions(id) ON DELETE SET NULL;

-- 2. Update the RPC function to handle the new payload and validate canonical identity
...
begin
  ...
  for v_item in select * from jsonb_array_elements(coalesce(p_result_breakdown->'perQuestion', '[]'::jsonb)) loop
    v_exam_question_id := v_item->>'examQuestionId';
