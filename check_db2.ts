...
async function check() {
  ...
  const { data: attemptData } = await supabase.from('cbt_attempts').select('*').eq('test_id', testId).order('created_at', { ascending: false }).limit(1).single();
  console.log("== cbt_attempts ==");
