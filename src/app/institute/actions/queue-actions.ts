...
export async function fetchQueueItems(instituteId: string, filter: string) {
  ...
  let query = supabase.from("solution_generation_queue").select("*, questions(question_text)").eq("institute_id", instituteId).order("updated_at", { ascending: false });
...
export async function regenerateFailed(instituteId: string) {
  ...
  const { data: failed } = await supabase.from("solution_generation_queue").select("question_id").eq("institute_id", instituteId).eq("status", "failed");
  
  if (!failed || failed.length === 0) return 0;

  for (const row of failed) {
    await supabase.from("solution_generation_queue")
