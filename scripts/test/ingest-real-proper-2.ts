...
async function main() {
  ...
  // 1. Read Answer Key
  const ansKeyPath = "C:\\Users\\SOURAV\\Documents\\examgrid data demo\\jee main 2025 (23 shift1)\\JEE Main 2025 (23 Jan Shift 1) answerkey.txt";
  ...
  const responsePath = "C:\\Users\\SOURAV\\Documents\\examgrid data demo\\23 shift student response.txt";
  ...
  const cropsPath = path.join(process.cwd(), 'public', 'uploads', 'cbt_assets', 'real_exam_2', 'crops_meta.json');
  ...
  const { error: eErr } = await supabase.from('exams').insert({
    ...
    title: 'JEE Main 2025 (23 Jan Shift 1) - Production Validation',
    exam_type: 'JEE_MAIN',
  ...
  for (let i = 0; i < crops.length; i++) {
    ...
    const { error: qErr } = await supabase.from('questions').insert({
      ...
      metadata: { published_image_url: q.filename ? `/uploads/cbt_assets/real_exam_2/${q.filename}` : null }
    });
    if (qErr) {
    ...
    publishedEqRows.push({
      ...
      published_image_url: q.filename ? `/uploads/cbt_assets/real_exam_2/${q.filename}` : null
    });
  }
